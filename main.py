...
label_temp.set_pos(10, 96)
label_temp.set_style_text_color(C_TEXT, 0)
label_temp.set_style_text_font(lv.font_montserrat_16, 0)

label_proc = lv.label(scr)
label_proc.set_text("PROC  ---")
label_proc.set_pos(10, 112)
label_proc.set_style_text_color(C_TEXT, 0)
label_proc.set_style_text_font(lv.font_montserrat_16, 0)

label_net_dl = lv.label(scr)
label_net_dl.set_text("DL  ---.-K/s")
label_net_dl.set_pos(10, 140)
...
sep1.set_text("--------------------------------")
sep1.set_pos(4, 52)
sep1.set_style_text_color(C_SEP, 0)
sep1.set_style_text_font(lv.font_montserrat_12, 0)

sep2 = lv.label(scr)
sep2.set_text("--------------------------------")
sep2.set_pos(4, 130)
sep2.set_style_text_color(C_SEP, 0)
...
while True:
    if select.select([sys.stdin], [], [], 0.05)[0]:
        ...
        try:
            ...
            color = C_CPU if cpu_pct < 70 else (C_ORANGE if cpu_pct < 90 else C_TEMP)
            ...
            color = C_RAM if ram_pct < 80 else (C_ORANGE if ram_pct < 95 else C_TEMP)
            ...
            label_temp.set_text("TEMP  " + (stats['temp'] if stats['temp'] != "N/A" else "N/A"))
            label_proc.set_text("PROC  " + str(stats['proc']))
